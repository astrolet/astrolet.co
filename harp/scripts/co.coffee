# Here put shared scripts -- made available to the layout-using pages and apps.

window.onload = ->

  # Underscore mixin helpers
  _.mixin
    ...
    raiconize: (opts) ->
      ...
      for id, icon of opts.icons
        ...
        unless conf.skip?
          ...
          if ($.find "##{id}").length > 0 # just in case there are more / non-matching icons
            $("##{id}").text("")
          ...
          $("##{id}").remove()
        #else: a false skip let's stuff remain as is (icon ignored)
  ...
  # This is cool, but is not seen - perhaps Typekit fonts should be loaded here?
  ...
  offset = spinOpts.radius + spinOpts.length
  $(target).css
  ...
  # Fill the icons with orange on hover, back to yellow on out.
  $("#social a").hover (-> $(@).find("path").attr "fill", "orange"),
                       (-> $(@).find("path").attr "fill", "#FFFF75")
